...
impl Drop for AppsLookupClient {
    fn drop(&mut self) {
        self.close();
    }
}
...
pub struct Handler {
    pub handle: Option<AppsLookupHandler>,
}
...
impl ManagedServer {
    ...
    pub fn with_workers(
        ...
        let raw_handler = handler.handle.map(raw::apps_lookup_dispatch);
        Self {
...
mod tests {
    ...
    fn managed_server_initializes_stopped_with_handler() {
        ...
        let server = ManagedServer::with_workers(
            "/tmp/netipc-apps-lookup-test",
            "apps-lookup-facade-test",
