...
use crate::protocol::{
    ...
    string_reverse_encode, AppsLookupBuilder, AppsLookupRequestView, AppsLookupResponseView,
    BatchBuilder, CgroupsLookupBuilder, CgroupsLookupRequestView, CgroupsLookupResponseView,
    CgroupsRequest, CgroupsResponseView, Header, NipcError, APPS_LOOKUP_KEY_SIZE,
    APPS_LOOKUP_REQ_HDR_SIZE, CGROUPS_LOOKUP_REQ_HDR_SIZE, FLAG_BATCH, HEADER_SIZE,
    INCREMENT_PAYLOAD_SIZE, KIND_REQUEST, KIND_RESPONSE, LOOKUP_DIR_ENTRY_SIZE, MAGIC_MSG,
    MAX_PAYLOAD_CAP, MAX_PAYLOAD_DEFAULT, METHOD_APPS_LOOKUP, METHOD_CGROUPS_LOOKUP,
    METHOD_CGROUPS_SNAPSHOT, METHOD_INCREMENT, METHOD_STRING_REVERSE, STATUS_BAD_ENVELOPE,
    STATUS_INTERNAL_ERROR, STATUS_LIMIT_EXCEEDED, STATUS_OK, STRING_REVERSE_HDR_SIZE, VERSION,
};
...
impl RawClient {
    ...
    pub fn call_cgroups_lookup(
        ...
        let req_len = protocol::encode_cgroups_lookup_request(paths, req_buf)?;
        let response = self.raw_call_with_retry_request_buf(METHOD_CGROUPS_LOOKUP, req_len)?;
    ...
    pub fn call_apps_lookup(
        ...
        let req_size = APPS_LOOKUP_REQ_HDR_SIZE
            ...
            .ok_or(NipcError::Overflow)?;
        let req_buf = ensure_client_scratch(&mut self.request_buf, req_size);
        let req_len = protocol::encode_apps_lookup_request(pids, req_buf)?;
        let response = self.raw_call_with_retry_request_buf(METHOD_APPS_LOOKUP, req_len)?;
