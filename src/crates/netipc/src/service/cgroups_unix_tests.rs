...
fn test_snapshot_handler_rejects_bad_request_metadata() {
    let handlers = test_cgroups_handlers();
    let on_snapshot = handlers.on_snapshot.expect("snapshot handler");
...
fn test_increment_batch_retry_on_failure() {
    ...
    let mut server1 = TestServer::start_with(svc, batch_server_config(), pingpong_handlers(), 8);
    ...
    let mut server2 = TestServer::start_with(svc, batch_server_config(), pingpong_handlers(), 8);
