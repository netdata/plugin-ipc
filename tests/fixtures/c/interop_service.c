...
static bool counting_handler(void *user,
                               uint16_t method_code,
                               const uint8_t *request_payload,
                               size_t request_len,
                               uint8_t *response_buf,
                               ...
                               size_t *response_len_out)
{
    bool ok = test_handler(user, method_code, request_payload, request_len,
        ...
        if (err != NIPC_OK) {
            ...
            for (int i = 0; i < 3; i++) {
                if (batch_out[i] != expected[i]) {
                    fprintf(stderr, "client: batch[%d] expected %lu, got %lu\n",
                            i, (unsigned long)expected[i], (unsigned long)batch_out[i]);
                    ok = 0;
                }
            }
        }
