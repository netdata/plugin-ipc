...
check_floors() {
    ...
    done < <(grep "^shm-ping-pong," "$INPUT_CSV" | head -4)

    # Named Pipe ping-pong max: documented (no mandatory floor)
    # Still check that it's non-zero
    while IFS=',' read -r scenario client server throughput p50 p95 p99 ccpu scpu tcpu; do
        local tp_int
        tp_int=$(printf "%.0f" "$throughput" 2>/dev/null || echo "0")
        if [ "$tp_int" -eq 0 ]; then
            warn "ZERO THROUGHPUT: np-ping-pong ${client}->${server}"
            violations=$((violations + 1))
        fi
    done < <(grep "^np-ping-pong," "$INPUT_CSV" | head -4)
...
generate_md() {
...

    {
        ...
        echo "## Named Pipe Ping-Pong (max throughput)"
        echo ""
        echo "| Client | Server | Throughput | p50 (us) | p95 (us) | p99 (us) | Client CPU | Server CPU |"
        echo "|--------|--------|-----------|----------|----------|----------|------------|------------|"

        grep "^np-ping-pong," "$INPUT_CSV" | head -4 | while IFS=',' read -r scenario client server throughput p50 p95 p99 ccpu scpu tcpu; do
            local tp_fmt
        ...
        # --- Win SHM Ping-Pong ---
        echo "## Win SHM Ping-Pong (max throughput)"
        echo ""
        echo "| Client | Server | Throughput | p50 (us) | p95 (us) | p99 (us) | Client CPU | Server CPU |"
        echo "|--------|--------|-----------|----------|----------|----------|------------|------------|"

        grep "^shm-ping-pong," "$INPUT_CSV" | head -4 | while IFS=',' read -r scenario client server throughput p50 p95 p99 ccpu scpu tcpu; do
            local tp_fmt
            tp_fmt=$(format_throughput "$throughput")
            printf "| %-6s | %-6s | %9s | %8s | %8s | %8s | %10s%% | %10s%% |\n" \
                "$client" "$server" "$tp_fmt" "$p50" "$p95" "$p99" "$ccpu" "$scpu"
        ...
        # --- Snapshot Named Pipe ---
        echo "## Snapshot Named Pipe Refresh (max throughput)"
        echo ""
        echo "| Client | Server | Throughput | p50 (us) | p95 (us) | p99 (us) | Client CPU | Server CPU |"
        echo "|--------|--------|-----------|----------|----------|----------|------------|------------|"

        grep "^snapshot-baseline," "$INPUT_CSV" | head -4 | while IFS=',' read -r scenario client server throughput p50 p95 p99 ccpu scpu tcpu; do
            local tp_fmt
            tp_fmt=$(format_throughput "$throughput")
            printf "| %-6s | %-6s | %9s | %8s | %8s | %8s | %10s%% | %10s%% |\n" \
                "$client" "$server" "$tp_fmt" "$p50" "$p95" "$p99" "$ccpu" "$scpu"
        ...
        # --- Snapshot Win SHM ---
        echo "## Snapshot Win SHM Refresh (max throughput)"
        echo ""
        echo "| Client | Server | Throughput | p50 (us) | p95 (us) | p99 (us) | Client CPU | Server CPU |"
        echo "|--------|--------|-----------|----------|----------|----------|------------|------------|"

        grep "^snapshot-shm," "$INPUT_CSV" | head -4 | while IFS=',' read -r scenario client server throughput p50 p95 p99 ccpu scpu tcpu; do
            local tp_fmt
            tp_fmt=$(format_throughput "$throughput")
            printf "| %-6s | %-6s | %9s | %8s | %8s | %8s | %10s%% | %10s%% |\n" \
                "$client" "$server" "$tp_fmt" "$p50" "$p95" "$p99" "$ccpu" "$scpu"
        ...
        local shm_ok="PASS" np_ok="PASS" lookup_ok="PASS"
        ...
        echo "| Win SHM ping-pong max | >= 1M req/s | $shm_ok |"
        echo "| Named Pipe ping-pong max | > 0 (baseline) | $np_ok |"
